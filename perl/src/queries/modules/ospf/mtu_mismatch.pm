...
sub check_mtu_mismatch {
    ...
    print STDERR "\nVerifying that there are no MTU mismatches for each OSPF adjacency.\n" if !$quiet;
	...
	if ($origin_mtu != $dest_mtu) {
	    if (!$quiet) {
		print STDERR "WARNING: MTU Mismatch for $origin_router => $dest_router.\n";
		print STDERR "Conflicting Interfaces: $origin_router : $origin_intf: MTU = $origin_mtu.\n";
		print STDERR "Conflicting Interfaces: $dest_router : $dest_intf: MTU = $dest_mtu.\n";
	    }
	}
